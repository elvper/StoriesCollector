...
function process_story (e, type) {
	...
	var character = (id, t, ranges, img) =>
		type == "forum" ?
			"[color=" + speaker_color + "]" + get_name(id) + "[/color]: " +
			text_part(t, ranges) + b
		: type == "docs" ?
			'<img src="' + img.slice(0, -4) +
			'.png" style="height:24px;width:18px;"><font color="' +
			speaker_color + '">' + get_name(id) + ":</font> " +
			text_part(t, ranges) + b
		:
			"";
	...
	var match = (lst) =>
		...
		: type == "docs" ?
	    	...
	    	b + '<table border="1"><col width="300"><col width="300"><tr>' +
			shuffle(lst.flatMap(ele => ele.phrase)).map((t, i) => "<td>" + t + "</td><td>" + lst[i].translation + "</td>").join("</tr><tr>") + "</tr></table>" + b
		:
			"";
	...
	var audiolink = (t) =>
		type == "forum" ?
			...
			"](https://stories-cdn.duolingo.com/audio/" +
	...
	txt +=
		...
		: type == "docs" ?
			...
			'</h2><table border="1"><col width="300"><col width="300"><tr><th>' +
			learning + "</th><th>" +
			from_language + "</th></tr>"
		:
			"";
	...
	for (var set of set_list.sets) {
		...
		for (var story of set) {
			...
			if (!(story.illustrationUrls.active.substr(39,40) in icons)){
				...
				console.log(story.illustrationUrls.active);
			}
